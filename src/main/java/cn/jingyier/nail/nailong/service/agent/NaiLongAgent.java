...
@Service
public class NaiLongAgent {

    private static final Logger log = LoggerFactory.getLogger(NaiLongAgent.class);
    ...
    public NaiLongAgent(ChatModel chatModel,
                        ConversationService conversationService,
                        MessageService messageService,
                        ObjectMapper objectMapper,
                        @Value("${nailong.agent.max-iterations:10}") int maxIterations) {
        ...
        try {
            ...
            try {
                writeEvent(out, "error", Map.of(
                        "type", "error",
                        "message", NaiLongPrompts.errorMessage(),
                        "characterState", CharacterStateMachine.IDLE.getCode()
                ));
    ...
    private void runAgentLoop(String sessionKey, String userContent, OutputStream out) throws IOException {
        ...
        writeEvent(out, "thinking", Map.of(
                "type", "thinking",
                "content", "嗯，在听。",
                "characterState", CharacterStateMachine.LISTENING.getCode()
        ));

        writeEvent(out, "thinking", Map.of(
                "type", "thinking",
                "content", "让奶龙看看...",
                "characterState", CharacterStateMachine.THINKING.getCode()
        ));
                            ...
                            if (content != null && !content.isEmpty()) {
                                ...
                                try {
                                    writeEvent(out, "text", Map.of(
                                            "type", "text",
                                            "content", content,
                                            "characterState", CharacterStateMachine.SPEAKING.getCode()
                                    ));
                                } catch (IOException e) {
                                    throw new RuntimeException(e);
                                }
        ...
        try {
            ...
            String finalState = isLaughTrigger(finalAnswer)
                    ? CharacterStateMachine.LAUGHING.getCode()
                    : CharacterStateMachine.IDLE.getCode();
    ...
    private List<org.springframework.ai.chat.messages.Message> buildContext(Long convId, String userContent) {
        ...
        messages.add(new SystemMessage(NaiLongPrompts.SYSTEM_PROMPT));

        List<Message> history = messageService.getHistoryAsMessages(convId);
    ...
    private List<String> generateRecommendations(String question) {
        ...
        if (q.contains("天气")) recs.add("明天天气怎么样？");
        if (q.contains("新闻")) recs.add("最近有什么新鲜事？");
        if (q.contains("代码") || q.contains("java") || q.contains("python")) recs.add("再推荐一个编程技巧？");
        recs.add("奶龙你会打瞌睡吗？");
        recs.add("讲个冷笑话来听听~");
        return recs.subList(0, Math.min(3, recs.size()));
    }

    private boolean isLaughTrigger(String content) {
        if (content == null) return false;
        return content.contains("哈哈") || content.contains("笑")
                || content.contains("嘿嘿") || content.contains("滑稽")
                || content.contains("😄") || content.contains("😂")
                || content.contains("好玩") || content.contains("有趣");
    }
}